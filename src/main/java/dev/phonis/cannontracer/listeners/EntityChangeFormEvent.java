...
public
class EntityChangeFormEvent implements Listener
{
    private final Tick tick;

    public
    EntityChangeFormEvent(JavaPlugin plugin, Tick tick)
    {
    ...
    @EventHandler
    public
    void onExplosionEvent(ExplosionPrimeEvent event)
    {
    ...
    @EventHandler
    public
    void onEntityBlockFormEvent(EntityChangeBlockEvent event)
    {
    ...
    private static
    void trackLastTick(Entity entity, Tick tick)
    {
        ...
        if (el == null)
        {
            change = new LocationChange(entity.getWorld(), loc, loc, entity.getType(), ChangeType.END,
                entity.getVelocity().length());
        }
        else
        {
            change = new LocationChange(entity.getWorld(), el.getLocation(), loc, entity.getType(), ChangeType.END,
                entity.getVelocity().length());
        }
