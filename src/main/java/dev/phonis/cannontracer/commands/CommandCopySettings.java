...
public
class CommandCopySettings extends EntityTracerCommand
{
    ...
    void execute(Player player, String[] args) throws CommandException
    {
        if (args.length > 0)
        {
            ...
            if (otherPlayer != null)
            {
                ...
                player.sendMessage(
                    ChatColor.BLUE + "Changing your settings to: \n" + ChatColor.RESET + tu.printSettings());

                otherPlayer.sendMessage(ChatColor.AQUA + player.getName() + " has taken your settings");
            }
            else
            {
